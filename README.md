...
Сервер использует базу данных для хранения ключей, в данном случае была выбрана [PostgreSQL]([https://www.postgresql.org/](https://www.postgresql.org/)), в качестве ORM системы для работы с БД используется [Gorm]([http://gorm.io/](http://gorm.io/)).
Сервер написан на языке программирования [Go]([https://golang.org/](https://golang.org/)), c использованием [модульной системы](https://github.com/golang/go/wiki/Modules) для загрузки зависимостей. 
...
1. **Web-сервер** для управление поступившими запросами.  Описание доступных url адресов страниц см. в таблице Web server.
...
Для осуществления работы с базой данных, используется [docker]([https://www.docker.com/](https://www.docker.com/).
Перед запуском сервера необходимо запустить докер.

#### Запуск докера Как установить и запустить Docker:
...
Для конфигурирования сервера, используется файл settins.json (json формат [RFC7159]([https://tools.ietf.org/html/rfc7159](https://tools.ietf.org/html/rfc7159))), 
...
### Таблицы в БД Для работы сервиса необходима 1 таблица `keys` SQL запрос для создания таблицы:
```sql
...
Для формы авторизации [jquery 3.3](https://jquery.com/).
...
Клиент осуществляет доступ к серверу через **websoket** подключение _/st_cpu_  В желтом окошечке у клиента ведётся лог подключения он сессионный, то есть если закрыть полностью окно браузере он сотрётся. Это сделано из-за ограничения размера cookies
