...
Серверная часть проекта решает задачу регистрации ключей на стороне сервера и управления ими через административную часть.
...
200 |Офлайн, отправляется клиентом когда активный клиент отключился, по собственной причине или по причине блокировки в момент его активного подключения. Этот статус нужен для того чтобы администраторы могли корректно отображать секцию online.
...
Публичная часть сервера(css, js, images и т.д.) находится в папке `cmd/server/http_files/public`

Статика для (html страницы) хранится: `cmd/server/http_files/` пользователям не получить к ней доступ напрямую.
...
/client | веб клиент для подключения к серверу. Также можно открывать: server/http_files/client.html в браузере.

#### RestAPi
 Url | Описание
---|---
/v1/login | Авторизация, отправляем логин и пароль POST запросом для авторизации(переменные: login, pass). Логин: admin Пароль: admin
/v1/logout | Разлогинится.

#### Websocket

Клиент сервер общаются в формате json, за исключением стандартных кодов ответов и описания к ним.

 Url | Описание
---|--- 
/st_cpu | Отправить ключ, и уже после одобрения в админке ключа можно подключиться и получать загрузку CPU. При запросе есть обязательный параметр Key.
/ws_list | Подключение для работы со списком запросов. Необходима авторизация.
    
#### Описание Запрос/Ответ: /st_cpu

##### Запрос от клиента на регистрацию, и получение значения загрузки ЦП.
...
```
Ответ:
Ответом будет либо статусное сообщение о закрытии подключения по причине того что ключ не прошел модерацию или забанен, либо ответ:
```json
{
  "CPU": 12.23
}
...
#### Описание Запрос/Ответ: /ws_list

Данный тип подключения доступен только для администраторов. Существует 2 формата.

##### Запрос на получения списков

В status передается номера статусов в соответсвии с таблицей статусов.
...
##### Запрос на обновление ключа

Запрос на обновление конкретного ключа. 
Ответ с типом newKey приходит всем администраторам, что бы они могли обновить свои списки.
...
![login form](/docs/images/admin_login.png)
...
![list](/docs/images/admin_list.png)

1. **Запросы** в эту категорию попадают ключи, которым администратор ещё не присвоил статус.
2. **Разрешенные** список ключей которым администратор разрешил доступ.
...
![accepted keys](/docs/images/admin_accepted_keys.png)
...
![popup](/docs/images/admin_popup.png)
...
При запуске клиента генерируется ключ и сохраняется в cookies на 1 год, если до этого был сгенерирован загружается уже сгенерированый ключ.

![client](/docs/images/client_firstconn.png)
...
![client](/docs/images/client_connected.png)
...
![client](/docs/images/client_banned.png)
